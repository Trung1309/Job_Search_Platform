...
class JobController extends Controller
{
    ...
    public function filterJob(Request $request){
        $query = Job::query();
        if($request->filled('id_the_loai')){
            $query->where('id_the_loai', $request->id_the_loai);

        }
        if($request->filled('id_vi_tri')){
            $query->where('id_vi_tri', $request->id_vi_tri);

        }
        if($request->filled('id_trinh_do')){
            $query->where('id_trinh_do', $request->id_trinh_do);

        }
        if($request->filled('id_kinh_nghiem')){
            $query->where('id_kinh_nghiem', $request->id_kinh_nghiem);

        }
        if ($request->filled('id_tinh_thanh')) {
            $query->whereHas('wards.districts.provinces', function($q) use ($request) {
