            ...
            <div class="main-page">
                <div class="charts-grids" style="display: flex; align-items: center ;justify-content: center; flex-direction: column" >
                    ...
                    <div class="form-schedule charts-grids" >
                        ...
                        <form action="{{ route('acceptMember', $member->id_ung_vien) }}" method="POST">
                            @csrf
                            <h1 style="padding: 20px 0px" class="text-center">Đặt lịch phỏng vấn</h1>
                            <p>Tên ứng viên: {{$member->users->ho_ten}}</p>
                            <p>Email: {{$member->users->email}}</p>
                            <p>Công việc: {{$member->jobs->ten_cong_viec}}</p>
                            <div class="form-group">

                                <label for="">Ngày phỏng vấn</label>
                                <input name="ngay_phong_van" type="datetime-local" class="form-control" placeholder="">
                                @error('ngay_phong_van')
